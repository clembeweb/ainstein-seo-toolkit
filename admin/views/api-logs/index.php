<?php
$stats = $stats ?? ['last_24h' => [], 'last_30d' => [], 'by_provider' => []];
$filters = $filters ?? [];
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <form method="POST" action="<?= url('/admin/api-logs/cleanup') ?>"
              onsubmit="return confirm('Eliminare i log più vecchi di 30 giorni?')">
            <?= csrf_field() ?>
    ...
    <!-- Provider Breakdown (se ci sono dati) -->
    <?php if (!empty($stats['by_provider'])): ?>
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-4">
        <h3 class="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Breakdown per Provider (30 giorni)</h3>
        <div class="flex flex-wrap gap-4">
            <?php foreach ($stats['by_provider'] as $provider): ?>
                <div class="flex items-center gap-2 text-sm">
                    <span class="font-medium <?= $providerColors[$provider['provider']] ?? 'text-slate-600 dark:text-slate-400' ?>">
                        <?= $providerNames[$provider['provider']] ?? $provider['provider'] ?>
                    </span>
                    <span class="text-slate-500 dark:text-slate-400">
                        <?= number_format($provider['calls']) ?> chiamate
                        <?php if ($provider['cost'] > 0): ?>
                            / $<?= number_format($provider['cost'], 4) ?>
                        <?php endif; ?>
                        <?php if ($provider['errors'] > 0): ?>
                            <span class="text-red-500">/ <?= $provider['errors'] ?> errori</span>
                        <?php endif; ?>
                    </span>
                </div>
            <?php endforeach; ?>
