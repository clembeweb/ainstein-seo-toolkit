    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
                ...
                <div class="p-5">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-medium text-slate-900 dark:text-white truncate ml-2"><?= e($project['gsc_property'] ?? '-') ?></span>
                        </div>
                ...
                <div class="p-5">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-medium text-slate-900 dark:text-white"><?= e($project['ga4_property_id'] ?? '-') ?></span>
                        </div>
                    ...
                    <p class="mt-3 text-xs text-slate-500 dark:text-slate-400">
                        Richiede Service Account JSON con accesso alla property GA4.
                    </p>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                <h3 class="font-medium text-slate-900 dark:text-white mb-4">Stato sincronizzazione</h3>
                <div class="space-y-3">
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-slate-500 dark:text-slate-400">Stato:</span>
                        ...
                        <?php elseif ($project['sync_status'] === 'completed'): ?>
                        <span class="text-emerald-600 dark:text-emerald-400">Completato</span>
                        <?php elseif ($project['sync_status'] === 'failed'): ?>
                        <span class="text-red-600 dark:text-red-400">Fallito</span>
                        <?php else: ?>
                        <span class="text-slate-600 dark:text-slate-400">In attesa</span>
                        <?php endif; ?>
                    </div>
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-slate-500 dark:text-slate-400">Ultimo sync:</span>
                        <span class="text-slate-900 dark:text-white">
                            <?= $project['last_sync_at'] ? date('d/m/Y H:i', strtotime($project['last_sync_at'])) : 'Mai' ?>
                        </span>
                    </div>
                </div>
                <?php if ($project['gsc_connected'] || $project['ga4_connected']): ?>
                <form action="<?= url('/seo-tracking/projects/' . $project['id'] . '/gsc/sync') ?>" method="POST" class="mt-4">
                    <input type="hidden" name="_csrf_token" value="<?= csrf_token() ?>">
                    <button type="submit" class="w-full px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-medium hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors text-sm" <?= $project['sync_status'] === 'running' ? 'disabled' : '' ?>>
                        <?= $project['sync_status'] === 'running' ? 'Sync in corso...' : 'Sincronizza ora' ?>
                    </button>
                </form>
                <?php endif; ?>
            </div>
        ...
        <div class="lg:col-span-2">
            ...
            <div class="mt-6 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-red-200 dark:border-red-900/50 overflow-hidden">
                <div class="p-6">
                    ...
                    <form action="<?= url('/seo-tracking/projects/' . $project['id']) ?>" method="POST" onsubmit="return confirm('Sei sicuro di voler eliminare questo progetto? Tutti i dati verranno persi.');">
                        <input type="hidden" name="_csrf_token" value="<?= csrf_token() ?>">
