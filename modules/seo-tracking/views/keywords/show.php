    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-slate-50 dark:bg-slate-800/50">
                    <tr>
                        <th class="px-4 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400">URL</th>
                        <th class="px-4 py-2 text-right text-xs font-medium text-slate-500 dark:text-slate-400">Click</th>
                        <th class="px-4 py-2 text-right text-xs font-medium text-slate-500 dark:text-slate-400">Impressioni</th>
                        <th class="px-4 py-2 text-right text-xs font-medium text-slate-500 dark:text-slate-400">Posizione</th>
                        <th class="px-4 py-2 text-right text-xs font-medium text-slate-500 dark:text-slate-400">CTR</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    <?php foreach ($gscTopPages as $page): ?>
                    <tr>
                        <td class="px-4 py-2 text-sm">
                            <a href="<?= e($page['page']) ?>" target="_blank" class="text-primary-600 dark:text-primary-400 hover:underline break-all block max-w-xs truncate" title="<?= e($page['page']) ?>">
                                <?= e(str_replace(['https://', 'http://'], '', $page['page'])) ?>
                            </a>
                        </td>
                        <td class="px-4 py-2 text-sm text-right font-semibold text-slate-900 dark:text-white"><?= number_format($page['total_clicks']) ?></td>
                        <td class="px-4 py-2 text-sm text-right text-slate-500 dark:text-slate-400"><?= number_format($page['total_impressions']) ?></td>
                        <td class="px-4 py-2 text-sm text-right text-slate-900 dark:text-white"><?= number_format($page['avg_position'], 1) ?></td>
                        <td class="px-4 py-2 text-sm text-right text-slate-500 dark:text-slate-400"><?= number_format(($page['avg_ctr'] ?? 0) * 100, 1) ?>%</td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-slate-50 dark:bg-slate-800/50">
                    <tr>
                        <th class="px-4 py-2 text-left text-xs font-medium text-slate-500 dark:text-slate-400">Paese</th>
                        <th class="px-4 py-2 text-right text-xs font-medium text-slate-500 dark:text-slate-400">Click</th>
                        <th class="px-4 py-2 text-right text-xs font-medium text-slate-500 dark:text-slate-400">Impressioni</th>
                        <th class="px-4 py-2 text-right text-xs font-medium text-slate-500 dark:text-slate-400">Posizione</th>
                    </tr>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    <?php foreach ($gscCountries as $country): ?>
                    <tr>
                        <td class="px-4 py-2 text-sm">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300">
                                <?= strtoupper(e($country['country'])) ?>
                            </span>
                        </td>
                        <td class="px-4 py-2 text-sm text-right font-semibold text-slate-900 dark:text-white"><?= number_format($country['total_clicks']) ?></td>
                        <td class="px-4 py-2 text-sm text-right text-slate-500 dark:text-slate-400"><?= number_format($country['total_impressions']) ?></td>
                        <td class="px-4 py-2 text-sm text-right text-slate-900 dark:text-white"><?= number_format($country['avg_position'], 1) ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
