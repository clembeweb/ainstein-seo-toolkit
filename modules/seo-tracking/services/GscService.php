...
class GscService
{
    ...
    public function getValidToken(int $projectId): ?string
    {
        ...
        if (strtotime($connection['token_expires_at']) < time()) {
            try {
                ...
                $this->gscConnection->updateTokens($projectId, [
    ...
    public function syncSearchAnalytics(int $projectId, ?string $startDate = null, ?string $endDate = null): array
    {
        ...
        try {
            ...
            // 1. Fetch dati per query (keyword)
            $queryData = $this->fetchSearchAnalytics($token, $connection['site_url'], $startDate, $endDate, ['query', 'date']);
            $result['records_fetched'] += count($queryData);
            ...
            // 2. Fetch dati per page
            $pageData = $this->fetchSearchAnalytics($token, $connection['site_url'], $startDate, $endDate, ['page', 'date']);
            ...
            $this->project->updateSyncStatus($projectId, 'completed', date('Y-m-d H:i:s'));
    ...
    private function updateTrackedKeywordPositions(int $projectId, string $startDate, string $endDate): void
    {
        ...
        foreach ($trackedKeywords as $kw) {
            ...
            if (!empty($positions)) {
                ...
                $this->keyword->updatePositionCache($kw['id'], $latest['position']);
            }
        }
    }
