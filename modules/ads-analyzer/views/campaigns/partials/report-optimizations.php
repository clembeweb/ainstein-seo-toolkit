...
<div x-data="{
    allOptimizations: <?= e(json_encode($allOptimizations ?? [], JSON_UNESCAPED_UNICODE)) ?>,
    ...
    get pendingGenCount() {
        ...
        for (const [key, opt] of Object.entries(this.allOptimizations)) {
            if (!this.generators[key]?.result && !<?= json_encode($manualOnlyTypes) ?>.includes(opt.type)) count++;
        }
        return count;
    },
    ...
    async executeBatchApply() {
        ...
        for (const key of selected) {
            ...
            // Skip PMax types — manual only
            if (<?= json_encode($pmaxTypes) ?>.includes(gen.type)) continue;
    ...
    getBeforeAfterData(key, opt) {
        ...
        // For rewrite_ads: extract before headlines/descriptions from campaignsData
        if (['rewrite_ads', 'rewrite_ad', 'copy'].includes(gen.type)) {
            let beforeAd = null;
            // Try to find the ad from campaignsData
            ...
            const afterData = gen.data;
            return {
                type: 'rewrite',
                before: beforeAd ? {
                    headlines: beforeAd.headlines || [],
                    descriptions: beforeAd.descriptions || []
                } : null,
                after: {
                    headlines: afterData.headlines || [],
                    descriptions: afterData.descriptions || []
                }
            };
        }
        ...
        if (['add_extensions', 'extensions'].includes(gen.type)) {
            return {
                type: 'extensions',
                items: gen.data.sitelinks || gen.data.callouts || gen.data.snippets || []
            };
        }

        // For PMax asset types
        ...
        if (['add_negatives', 'keywords'].includes(gen.type)) {
            return {
                type: 'negatives',
                keywords: gen.data.keywords || gen.data.negatives || []
            };
        }
