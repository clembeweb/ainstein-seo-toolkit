...
<div class="space-y-8">
    ...
    <!-- 2 Mode Cards -->
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6" data-tour="aa-modes">
        <!-- Keyword Negative -->
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-md transition-shadow">
            <div class="p-6">
                ...
                <h3 class="text-lg font-semibold text-slate-900 dark:text-white">Keyword Negative</h3>
                <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                    Carica CSV search terms da Google Ads, analizza con AI ed estrai keyword negative per ottimizzare le campagne.
                </p>
                <div class="mt-3 flex items-center gap-2">
                    ...
                    <?php if ($stats['negkw_count'] > 0): ?>
                    <span class="text-xs text-slate-400 dark:text-slate-500"><?= $stats['negkw_count'] ?> progetti</span>
                    <?php endif; ?>
                </div>
            </div>
            <div class="px-6 py-4 bg-slate-50 dark:bg-slate-700/50 border-t border-slate-200 dark:border-slate-700">
                <a href="<?= url('/ads-analyzer/projects?tab=negative-kw') ?>" class="inline-flex items-center text-sm font-medium text-amber-600 dark:text-amber-400 hover:text-amber-700">
                    Vai ai progetti
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-md transition-shadow">
            <div class="p-6">
                ...
                <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                    Collega Google Ads Script, raccogli dati sulle campagne e ottieni valutazioni AI con suggerimenti di ottimizzazione.
                </p>
                <div class="mt-3 flex items-center gap-2">
            ...
            <div class="px-6 py-4 bg-slate-50 dark:bg-slate-700/50 border-t border-slate-200 dark:border-slate-700">
                <a href="<?= url('/ads-analyzer/projects?tab=campaign') ?>" class="inline-flex items-center text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-700">
                    Vai ai progetti
                    <svg class="w-4 h-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                    </svg>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="divide-y divide-slate-200 dark:divide-slate-700">
                <?php foreach ($recentProjects as $project):
                    $isNegKw = ($project['type'] ?? 'negative-kw') === 'negative-kw';
                    $typeConfig = $isNegKw
                        ? [
