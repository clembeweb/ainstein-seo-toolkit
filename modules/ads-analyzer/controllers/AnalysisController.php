...
class AnalysisController
{
    ...
    public function extractContext(int $projectId, int $adGroupId): void
    {
        ...
        // Verifica crediti (3 per estrazione contesto)
        if (!Credits::hasEnough($user['id'], 3)) {
            jsonResponse(['error' => 'Crediti insufficienti (servono 3 crediti)'], 400);
            return;
        }
        ...
        Credits::consume($user['id'], 3, 'Estrazione contesto da landing page', 'ads-analyzer');
    ...
    public function extractAllContexts(int $projectId): void
    {
        ...
        $creditsNeeded = count($adGroupsWithUrl) * 3;
        if (!Credits::hasEnough($user['id'], $creditsNeeded)) {
        ...
        foreach ($adGroupsWithUrl as $adGroup) {
            ...
            if ($result['success']) {
                ...
                Credits::consume($user['id'], 3, "Estrazione contesto: {$adGroup['name']}", 'ads-analyzer');
