...
class Project
{
    ...
    public function findAccessible(int $id, ?int $userId = null): ?array
    {
        ...
        if ($role !== 'owner' && !\Services\ProjectAccessService::canAccessModule(
            (int)$project['global_project_id'], $userId, 'seo-onpage'
        )) {
            return null;
        }
    ...
    /**
     * Find project with stats, accessible by the user (owner or shared member).
     */
    public function findWithStatsAccessible(int $id, int $userId): ?array
    {
        // Fast path: direct owner
        $project = $this->findWithStats($id, $userId);
        if ($project) {
            $project['access_role'] = 'owner';
            return $project;
        }
        ...
        // Load stats for the shared project
