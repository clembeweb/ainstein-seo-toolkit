...
class GscService
{
    ...
    public function exchangeCode(string $code): array
    {
        ...
        // OAuth token endpoint richiede form-urlencoded
        $response = $this->httpPostForm(self::OAUTH_TOKEN_URL, $data);
    ...
    public function refreshTokenIfNeeded(array $connection): ?array
    {
        ...
        if (empty($refreshToken)) {
            return [
                'error' => true,
                'message' => 'Refresh token mancante. Riconnetti GSC.',
            ];
        }
        ...
        // OAuth token endpoint richiede form-urlencoded, non JSON!
        $response = $this->httpPostForm(self::OAUTH_TOKEN_URL, $data);

        if (isset($response['error'])) {
            return [
                'error' => true,
                'message' => $response['error_description'] ?? $response['error'] ?? 'Token refresh failed',
            ];
        }
        ...
        if (empty($response['access_token'])) {
            return [
                'error' => true,
                'message' => 'Risposta token non valida',
            ];
        }
    ...
    public function syncDailyData(int $projectId, int $days = 3): array
    {
        ...
        Database::insert('sa_gsc_sync_log', [
            ...
            'records_imported' => count($data['queries']) + count($data['pages']),
            'status' => 'success',
    ...
    public function saveConnection(int $projectId, int $userId, array $tokens, string $propertyUrl, string $propertyType): int
    {
        ...
        // Database::insert() returns the lastInsertId
    ...
    /**
     * HTTP POST request via shared ScraperService (JSON)
     */
    ...
    /**
     * HTTP POST form-urlencoded (per OAuth token endpoints)
     */
