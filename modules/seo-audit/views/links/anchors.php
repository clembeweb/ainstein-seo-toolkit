        ...
        <div class="overflow-x-auto max-h-96 overflow-y-auto">
            <table class="w-full text-sm">
                <thead class="bg-slate-50 dark:bg-slate-700/50 sticky top-0">
                    <tr>
                        <th class="px-6 py-3 text-left font-medium text-slate-500 dark:text-slate-400">Anchor Text</th>
                        <th class="px-6 py-3 text-center font-medium text-slate-500 dark:text-slate-400">Occorrenze</th>
                        <th class="px-6 py-3 text-center font-medium text-slate-500 dark:text-slate-400">Destinazioni</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    <?php foreach ($anchorData['top_anchors'] as $anchor): ?>
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                        <td class="px-6 py-2.5 text-slate-700 dark:text-slate-300 truncate max-w-xs"><?= e($anchor['anchor']) ?></td>
                        <td class="px-6 py-2.5 text-center font-medium text-slate-900 dark:text-white"><?= $anchor['count'] ?></td>
                        <td class="px-6 py-2.5 text-center text-slate-600 dark:text-slate-400"><?= $anchor['targets'] ?></td>
                    </tr>
                    <?php endforeach; ?>
                    <?php if (empty($anchorData['top_anchors'])): ?>
                    <tr><td colspan="3" class="px-6 py-8 text-center text-slate-500 dark:text-slate-400">Nessun dato disponibile</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
