<div class="space-y-6">
    ...
    <div>
        <nav class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 mb-2">
            ...
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-slate-900 dark:text-white">Google Search Console</span>
        </nav>
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Google Search Console</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Collega il tuo account Google per importare dati di performance</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                <h2 class="text-lg font-semibold text-slate-900 dark:text-white">Connesso a Google Search Console</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    Proprietà: <span class="font-medium text-slate-700 dark:text-slate-300"><?= e($connection['property_url']) ?></span>
                </p>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div class="p-8 text-center">
            <!-- Google Logo -->
            <div class="mx-auto w-20 h-20 rounded-full bg-gradient-to-br from-blue-500 via-red-500 to-yellow-500 p-0.5 mb-6">
            ...
            <h2 class="text-xl font-bold text-slate-900 dark:text-white mb-2">Connetti Google Search Console</h2>
            <p class="text-slate-500 dark:text-slate-400 mb-6 max-w-md mx-auto">
                Importa dati di performance, query di ricerca, pagine indicizzate e Core Web Vitals
                direttamente da Google Search Console.
            </p>
            ...
            <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 mb-6 max-w-md mx-auto">
                <div class="flex items-center gap-2 text-yellow-700 dark:text-yellow-300">
                    ...
                    <span class="text-sm font-medium">Modalità Demo</span>
                </div>
                <p class="text-sm text-yellow-600 dark:text-yellow-400 mt-1">
                    Credenziali Google non configurate. Verranno usati dati di esempio.
                </p>
            </div>
            <?php endif; ?>

            <a href="<?= e($authUrl) ?>" class="inline-flex items-center px-6 py-3 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700 transition-colors shadow-sm">
                <svg class="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                </svg>
                <?= $isMockMode ? 'Connetti (Demo)' : 'Accedi con Google' ?>
            </a>
        </div>

        <!-- Benefits -->
        <div class="px-8 py-6 bg-slate-50 dark:bg-slate-700/50 border-t border-slate-200 dark:border-slate-700">
            <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4">Cosa otterrai:</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-medium text-slate-900 dark:text-white">Performance</p>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Click, impressions, CTR, posizione</p>
                    </div>
                </div>
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-medium text-slate-900 dark:text-white">Query</p>
                        <p class="text-xs text-slate-500 dark:text-slate-400">Keyword che portano traffico</p>
                    </div>
                </div>
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-medium text-slate-900 dark:text-white">Core Web Vitals</p>
                        <p class="text-xs text-slate-500 dark:text-slate-400">LCP, INP, CLS metrics</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <?php if (!$isConfigured && !$isMockMode): ?>
    <!-- Admin Notice -->
    <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
        <div class="flex items-start gap-3">
            ...
            <div>
                <h3 class="text-sm font-semibold text-amber-800 dark:text-amber-200">Configurazione richiesta</h3>
                <p class="text-sm text-amber-700 dark:text-amber-300 mt-1">
                    L'amministratore deve configurare le credenziali Google OAuth nelle impostazioni del modulo SEO Audit.
                </p>
            </div>
        </div>
    </div>
