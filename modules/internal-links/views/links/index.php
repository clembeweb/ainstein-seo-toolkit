                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                <form action="<?= url("/internal-links/project/{$project['id']}/links/delete/{$link['id']}") ?>" method="POST" class="inline"
                                      onsubmit="return confirm('<?= __('Sei sicuro di voler eliminare questo link?') ?>')">
                                    <?= csrf_field() ?>
                                    <button type="submit"
...
function executeBulkAction() {
    ...
    if (selected.length === 0) {
        alert('<?= __('Seleziona almeno un link') ?>');
        return;
    }

    if (action === 'delete' && !confirm(`<?= __('Sei sicuro di voler eliminare') ?> ${selected.length} <?= __('link') ?>?`)) {
        return;
    }
