...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-center gap-2">
        ...
        <div class="flex items-center">
            <div class="flex items-center justify-center w-8 h-8 rounded-full text-sm font-medium <?= $step['active'] ? 'bg-primary-600 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400' ?>">
                <?= $step['num'] ?>
            </div>
            ...
            <form action="<?= url('/ai-optimizer/project/' . $projectId . '/optimize') ?>" method="POST" class="space-y-5">
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <button type="submit" <?= !$isConfigured ? 'disabled' : '' ?>
                            class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
